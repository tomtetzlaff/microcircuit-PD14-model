...
\hline
\drvd{$I_{\mathcal{C}}$} & \drvd{$\nu_{\mathcal{C}}\bar{I}\tauS = 0.3 \pA$} & mean current generated by a Poissonian input with rate $\nu_\mathcal{C}$, convolved with an exponential kernel of amplitude $\bar{I}$ and time constant $\tau_\text{s}$\\
  \hline
  %% TODO: here, we need to account for the fact that the $\bar{I}$'s are different for each target population (see synaptic weights)
\drvd{$I_{\mathcal{C}_y}$} & \drvd{$K_{\mathcal{C}_y}I_\mathcal{C}$} & mean total input current of cortico-cortical inputs ($\forall y\in\{\mathcal{E}_{23}, \dots, \mathcal{I}_{6}\}$)\\ 
